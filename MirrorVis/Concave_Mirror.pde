class Mirror {
  ...
  Mirror(PVector pos, float d) {
    this.pos = pos;
    this.setD(d);
    this.mainFocus = new PVector(pos.x + this.f, pos.y);
  }
  ...
  void setD(float d) {
    ...
    this.r = d/2;
    this.f = r/2;
    this.mainFocus = new PVector(this.pos.x + this.f, pos.y);
  }
  ...
  void show() {
    ...
    if (this.showFocus || this.showCenter) {
      fill(255);
      if (this.showFocus) {
        ellipse(this.pos.x + this.f, this.pos.y, 4, 4);
      }
      if (this.showCenter) {
  ...
  void reflect(Edge[] edges) {
    for (int i = 0; i < edges.length; i++) {
      ...
      float theta = asin(y / this.r);
      float x = cos(theta) * this.r; // x is `NaN` when the ray doesn't meet the mirror.
      x += this.pos.x;
      y = this.pos.y - y;
      ...
      if (str(x) != "NaN") {
        Ray poleRay = ray;
        poleRay.setReflectionPoint(new PVector(pos.x + this.r, pos.y));
